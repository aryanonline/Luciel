...
- **Security:** Free-tier CAPTCHA gate closes a P1 abuse surface. IAM scope expansion **was required** at Commit 2 — the initial preflight claim that "no new IAM scope expansion is expected" was **wrong**: the sandbox-agent `LucielSandboxArc5MigrateScope` inline policy had zero SSM Sids and zero KMS Sids (the Arc 5 surface was DB-migrate + ECS-deploy + RDS-snapshot only; it never touched Stripe credentials at the sandbox-agent layer — only the ECS task-execution-role does, and that's a different principal). The miss was caught by the Commit 2 first call to `boto3 ssm.get_parameter` returning `AccessDeniedException` BEFORE any destructive Stripe action. The 5-gate scope-expansion protocol was triggered; 3 new Sids (`Arc6SsmReadStripeProduction`, `Arc6SsmWriteStripeProduction`, `Arc6KmsViaSsmForStripeProduction`) were authored with tighter alternatives rejected, blast-radius analysis, and partner-verbatim approval, all recorded in `docs/runbooks/aws-sandbox-credential-posture.md` §8. The 3 new Sids land in a **customer-managed policy** named `LucielSandboxStripeScope` (ARN `arn:aws:iam::729005488042:policy/LucielSandboxStripeScope`) attached to the IAM user `luciel-sandbox-agent` — NOT appended to the Arc 5 `LucielSandboxArc5MigrateScope` inline policy (which would have exceeded the 2048-char IAM inline-user-policy size limit, 2640 chars total / 128.9%). Customer-managed was chosen over a second inline policy because: own version history, independently revocable without touching the Arc 5 policy, reusable on future principals, single canonical ARN for audit. The Arc 5 inline policy is byte-identical to its Arc 5 Commit 21 final form; the Arc 6 customer-managed policy is additive and independently revocable. Paste-ready JSON at `ops/iam/LucielSandboxStripeScope.json` (646 chars, 10.5% of the 6144-char customer-managed limit); delta view at `ops/iam/arc6_stripe_ssm_scope_expansion.json`. Partner created the policy via IAM Console at 2026-05-23 6:26 PM EDT; four `boto3 ssm` verification probes from the sandbox-agent at ≈22:26 UTC all passed (read on intro-fee Price ID; read+decrypt on stripe_secret_key; write+encrypt of a probe SecureString; round-trip read on the probe). The probe parameter `/luciel/production/stripe_arc6_iam_probe` is cleaned up at Commit 11 doctrine-close. Stripe Live ops run via the Stripe Live secret already in SSM at `/luciel/production/stripe_secret_key` since Step 30a.2-pilot GATE 3 (2026-05-15) — the new IAM Sids let the sandbox-agent **read** that secret to drive Stripe API calls during the Arc 6 wipe + mint windows.
