...
#### §3.6.X — Phase C execution record (2026-05-22 ~20:00–22:30 EDT)

**Phase C landed in prod 2026-05-22 ~20:00–22:30 EDT — functionally COMPLETE; operator-side mailbox confirmation + AWS sandbox-exit approval remain external dependencies.**

**Phase C ordered execution (preflight + 7 steps + 1 inline patch):**

- **Preflight** (2026-05-22 ~19:50 EDT). Pulled `722a945..aaf94f2` to ECR-side workspace; confirmed Alembic migration files `a91c4d2e7f08_arc8_wu6_email_send_event.py` and `b2e5f17a3d9c_arc8_wu6_email_suppression.py` on disk; AWS account `729005488042`, region `ca-central-1` locked in. HEAD at `aaf94f2` (commit `30b8af4` author of `app/api/v1/ses_events.py` route + 18 AST/text tests; commit `aaf94f2` author of the Phase C deploy runbook).
- **Step 1 — Discover migration absence in old image** (2026-05-22 ~20:05 EDT). ECS exec into the running `luciel-backend:81` task surfaced that the migration files were **not present in the deployed container** (`:81` predates commit `c3d974f`). Resequence decision: build first, migrate after — the new image must land before `alembic upgrade head` can be run inside an ECS-exec session.
- **Step 2 — `.dockerignore` patch + paranoid local image build** (2026-05-22 ~20:15 EDT). Patched `.dockerignore` with Phase C exclusions: `docs/`, `arc*-out/`, `iam-*.json`, `*.docx`, `~$*`, and doctrine `*.md` files at repo root. UTF-8 BOM-safe write (Standard #10) via `[IO.File]::WriteAllText`. `docker buildx build --platform linux/amd64 --provenance=false --sbom=false` (Standard #11) completed in 79s. Paranoid local gate (Standard #13) via `docker inspect` confirmed: PID 1 = `luciel` (uid `10001`), migration files present at `/app/alembic/versions/`, route file present at `/app/app/api/v1/ses_events.py` (497 lines), platform `linux/amd64`, no operational artifacts (`diag*.py`, `td-*.json`, `verify*.json`) baked into the image. Image digest captured: `sha256:ef7f8fe1c3a721d88ecd2f72d18770bd857efbb39cc92f1723b292ee930b6da3`.
- **Step 3 — ECR push with digest chain-of-custody** (2026-05-22 ~20:25 EDT). `docker push 729005488042.dkr.ecr.ca-central-1.amazonaws.com/luciel-backend:82` succeeded; post-push `aws ecr describe-images --image-ids imageTag=82` confirmed the manifest digest matched the locally-captured `sha256:ef7f8fe1...` byte-for-byte (Standard #13 chain-of-custody).
- **Step 4 — Task-def `:82` register + force-deploy backend** (2026-05-22 ~20:35 EDT). Task-def `luciel-backend:82` registered cloning `:81`'s shape with the only diff being the image reference pinned to the new digest. `aws ecs update-service --service luciel-backend-service --task-definition luciel-backend:82 --force-new-deployment` triggered the rolling deploy; old task drained and new task `f562274db75d4f05b0147ee4e3ba8c7a` on container instance `ip-10-0-10-63` reached `RUNNING` status. Deployment marked `COMPLETED`. Smoke test of the route (POST `/api/v1/ses-events` with empty body) returned `400 Malformed JSON` (correct: route is registered, JSON-decoder reachable); GET to the same path returned `405 Method Not Allowed` (correct: POST-only).
- **Step 4b — Worker symmetry (partner-prompted)** (2026-05-22 ~20:50 EDT). Partner flagged the worker symmetry gap ("partner dont we need to register and roll out the worker too"). Agent agreed — without a worker image refresh, any background-job code paths that rely on the new precheck/suppression helpers would silently miss them. Task-def `luciel-worker:37` registered with the same image digest; `update-service` force-deployed worker. Textbook clean rolling deploy observed in CloudWatch: both old and new worker heartbeats visible during overlap, warm shutdown of old worker after queue drain.
- **Step 5 — Migrations on prod RDS** (2026-05-22 ~21:05 EDT). ECS-exec into the new `:82` backend task (no `psql` or `curl` in the lean image; ran `python -m alembic upgrade head` from inside the container with the bound `DATABASE_URL` env). Migration chain applied cleanly: `b4d8a2e7c1f3` → `a91c4d2e7f08` → `b2e5f17a3d9c`. Schema verified via in-container `python -c "from sqlalchemy import inspect; ..."` (no `psql` in image); both `email_send_event` and `email_suppression` tables present with the expected constraints.
- **Step 5b — SSM parameter + task-def env-binding revision** (2026-05-22 ~21:25 EDT). Created SSM parameter `/luciel/production/SES_SNS_TOPIC_ARN` (Type=String, Tier=Standard) with value `arn:aws:sns:ca-central-1:729005488042:luciel-ses-events`. **Critical design discovery (partner-prompted):** Partner suggested agent read the repo directly ("you have access to my repo cant you look at it from your end"); agent used GitHub CLI to fetch `app/core/config.py` from origin/main and caught a runbook-vs-code naming bug: `app/core/config.py` declares `ses_sns_topic_arn: str = ""` with `model_config = SettingsConfigDict(env_file=".env", extra="ignore")` and **no `env_prefix`** set. Pattern matches existing `REDIS_URL`→`redis_url`, `MAGIC_LINK_SECRET`→`magic_link_secret`. Therefore the env var bound into the task def must be **`SES_SNS_TOPIC_ARN`** (NOT `LUCIEL_SES_SNS_TOPIC_ARN` as the original runbook prose said). Task-def `luciel-backend:83` registered with the corrected secret binding; `update-service` force-deployed; env var verification via ECS-exec confirmed `settings.ses_sns_topic_arn = 'arn:aws:sns:...:luciel-ses-events'`.
- **Step 6 — SNS subscription + auto-confirm** (2026-05-22 ~22:00 EDT). `aws sns subscribe --topic-arn arn:aws:sns:ca-central-1:729005488042:luciel-ses-events --protocol https --notification-endpoint https://www.vantagemind.ai/api/v1/ses-events`. The route auto-confirmed in ~1 second — CloudWatch log line `SubscriptionConfirmation confirmed ... subscribe_status=200` proves the route's `SubscriptionConfirmation` handler at `ses_events.py:line~140` correctly fetched the AWS-provided `SubscribeURL`, opened it via `urllib`, and returned 200. Confirmed subscription ARN: `arn:aws:sns:ca-central-1:729005488042:luciel-ses-events:f15b16e1-24ed-484a-9772-ba8fd5fcdb90`.
- **Step 6b — IAM gap surfaced + inline-patched** (2026-05-22 ~22:10–22:15 EDT). First synthetic-bounce test failed with `AccessDeniedException: ... is not authorized to perform: ses:SendEmail on resource: arn:aws:ses:ca-central-1:729005488042:configuration-set/luciel-default`. Root cause: Phase B's `LucielSESSendEmail` policy granted `ses:SendEmail` on `identity/*` only — pre-WU-6 code didn't pass `ConfigurationSetName`, so the gap was invisible; WU-6 code does, so the gap surfaced on first send. Inline patch applied atomically via `aws iam put-role-policy` extending the Resource array to `[identity/*, configuration-set/luciel-default]`. **Snapshot integrity:** only the post-patch policy state was captured to operator-local disk and is preserved in-repo at `iam/LucielSESSendEmail-post-patch-2026-05-22.json`; the pre-patch Resource array `[identity/*]` is asserted from operator memory of the patch command + the `~~D-ses-iam-overgrant-unused-actions-2026-05-22~~` closure stanza (which documents the Phase B rightshape to that exact single-ARN shape) and is recoverable from AWS CloudTrail `PutRolePolicy` event history (90-day retention). **Net-new drift logged + closed-in-same-arc:** `~~D-ses-iam-config-set-resource-missing-2026-05-22~~` (DRIFTS §3 entry; the closure stanza there carries the same Snapshot integrity note for the doctrine-honest pre/post-patch claim).
- **Step 7 — Operator-side mailbox confirmation (DEFERRED to a dedicated post-Arc-5/6 mini-arc, 2026-05-22 ~21:40 EDT)** — `vantagemind.ai` DNS is hosted on GoDaddy, not Cloudflare, so the originally-prescribed Cloudflare Email Routing path is not directly available; closing this drift requires either (a) a DNS migration from GoDaddy to Cloudflare followed by Email Routing enablement, (b) a GoDaddy-native email forwarder, or (c) a third-party forwarder (Improvmx / Forwardemail.net) via GoDaddy MX edits. Decision 2026-05-22 ~21:40 EDT: defer this entire decision to a dedicated mini-arc after Arc 5 (schema migration) and Arc 6 (Stripe SKU restructure) land, to avoid stacking DNS-layer risk against product-critical schema/billing changes. `D-ses-reply-to-monitored-inbox-not-confirmed-2026-05-22` remains explicitly OPEN at status CODE-DEPLOYED-AWAITING-MAILBOX-CONFIRM. The Arc 8 WU-6 closing tag `arc-8-wu-6-ses-feedback-loop-complete` is correspondingly deferred until this drift closes.

**End-to-end synthetic-bounce evidence (Step 6 verification, 2026-05-22 ~22:14 EDT):**

- Magic-link send invoked with `to=bounce@simulator.amazonses.com`. AWS SES returned MessageId `010d019e523ba0d7-e46f9a4d-e609-4df7-9133-d61e310ce2fa-000000`.
- SES emitted a `Bounce` event to configuration set `luciel-default` → event destination → SNS topic `luciel-ses-events` → HTTPS subscription → backend route at `https://www.vantagemind.ai/api/v1/ses-events`.
- Backend route's two-check trust gate (expected_topic = `settings.ses_sns_topic_arn`; actual_topic = message body) passed.
- DB row inserted: `email_send_event id=1 event_type=Bounce event_id=e3a2c23f-9c8a-5bdb-aade-a0b56a6eeeaa`.
- Suppression-write side-effect inserted: `email_suppression id=1 address=bounce@simulator.amazonses.com reason=HardBounce source_event_id=e3a2c23f-9c8a-5bdb-aade-a0b56a6eeeaa`.
- Audit-chain integrity verified: `suppression.source_event_id` exactly matches `event.event_id` (FK constraint enforced).

**Drift state after Phase C execution (mirror of DRIFTS §3 SES cohort, post-Phase-C):**

- ~~`D-ses-iam-overgrant-unused-actions-2026-05-22`~~ — RESOLVED at Phase B B3 (no change)
- ~~`D-ses-feedback-loop-not-wired-2026-05-22`~~ — **RESOLVED** at Phase C (synthetic bounce E2E proof landed)
- ~~`D-ses-suppression-app-layer-not-implemented-2026-05-22`~~ — **RESOLVED** at Phase C (precheck-record-suppress loop verified live)
- `D-ses-reply-to-monitored-inbox-not-confirmed-2026-05-22` — **CODE-DEPLOYED-AWAITING-MAILBOX-CONFIRM** (code leg live in prod; operator mailbox confirmation owes)
- `D-ses-sandbox-exit-pending-2026-05-22` — SUBMITTED-AWAITING-AWS (case `177948223100786`, no change)
- ~~`D-ses-iam-config-set-resource-missing-2026-05-22`~~ — **RESOLVED** in-arc at Phase C Step 6b (NEW drift, resolved-in-same-arc)

**New drifts logged at Phase C (Phase D hardening cohort, all OPEN):**

- `D-version-endpoint-auth-gated-2026-05-22` — `/version` requires auth; breaks deploy-gate observability pattern
- `D-task-def-no-healthcheck-2026-05-22` — neither `luciel-backend:83` nor `luciel-worker:37` carry a container-level `healthCheck` block
- `D-ssm-naming-mixed-prefixes-2026-05-22` — legacy `/luciel/<name>` flat-shape parameters coexist with new `/luciel/production/<NAME>` env-scoped shape; IAM least-privilege scoping ambiguous until consolidated
- `D-ecs-exec-role-overbroad-ssm-2026-05-22` — `luciel-ecs-task-execution-role` has `AmazonSSMReadOnlyAccess` managed policy attached as a workaround for the mixed-prefix gap; account-wide grant violates least-privilege

**Prod-state marker after Phase C (2026-05-22 ~22:30 EDT):**

- Backend: task-def `luciel-backend:83` (image digest `sha256:ef7f8fe1c3a721d88ecd2f72d18770bd857efbb39cc92f1723b292ee930b6da3`); running task `f562274db75d4f05b0147ee4e3ba8c7a` on container instance `ip-10-0-10-63`
- Worker: task-def `luciel-worker:37` (same image digest)
- DB HEAD: `b2e5f17a3d9c`
- SNS subscription ARN: `arn:aws:sns:ca-central-1:729005488042:luciel-ses-events:f15b16e1-24ed-484a-9772-ba8fd5fcdb90`
- IAM `LucielSESSendEmail` Resource array: `[identity/*, configuration-set/luciel-default]`
- SSM `/luciel/production/SES_SNS_TOPIC_ARN`: live, bound to backend task def as env var `SES_SNS_TOPIC_ARN`

**Operational rules carried (per the locked operating doctrine):**

- "When its time to touch prod we will do together step by step" — every prod-touching step in Phase C was walked synchronously with the operator's PowerShell, with the agent's role limited to runbook authoring + verification + ECS-exec command construction. The agent never executed AWS API calls directly.
- "we are designing a business we can get lazy and just ignore things" — the worker-symmetry catch (Step 4b), the env-var naming bug catch (Step 5b), and the IAM config-set gap surfacing (Step 6b) are all instances of the no-shortcut discipline producing the right outcome. None were originally in the runbook; all three would have produced production defects if shortcuts had been taken.
