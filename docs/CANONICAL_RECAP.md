...
| Frontend | **30b** | Embeddable chat widget that any company can drop into their existing website. | A company adds a few lines of code to their site, and within an hour their visitors are having real conversations with the company's Luciel. This is the unblock for the first paying customer. | 🔧 Build complete on `step-30b-chat-widget` (schema + endpoint + Preact bundle + e2e); CDN deploy and embed-key issuance workflow owed before success criterion is met |
