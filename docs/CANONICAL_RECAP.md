...
**Last updated:** 2026-05-11 (Step 24.5c implementation complete — v1 cross-channel identity & conversation continuity live)
...
| Identity | **24.5c** | Cross-channel identity and conversation continuity. | A user moving between channels (widget, phone, email) is recognized as the same person, and the conversation continues without reset. The v1 build proof uses the two reachable channels today — the embeddable chat widget and the programmatic API — exchanging messages on two different sessions under the same `conversation_id`, joined by one `identity_claims` row, with the cross-session retriever surfacing the sibling session's recent turns. The voice/SMS/email legs of the literal T8 demonstration (widget Monday → phone Wednesday) inherit the same primitives but require Step 34a's channel adapter framework to be reachable end-to-end, and stay scoped there per the T8 footnote in §13.1. | ✅ Implemented — v1 cross-channel identity & conversation continuity landed across five sub-branches on 2026-05-11 (PRs #24 models+migration, #25 `CrossSessionRetriever`, #26 `IdentityResolver`, #27 adapter claims wiring, #28 live e2e + doc-truthing). Closing tag: `step-24-5c-cross-channel-identity-complete` cut on this doc-truthing commit (same shape as Step 30c's `99c6eb5`). Live e2e proof in `tests/e2e/step_24_5c_live_e2e.py` against real Postgres; backend-free harness-shape pin in `tests/api/test_step24_5c_live_e2e_shape.py`. Design-locked on 2026-05-11 (the row's earlier `📋 Candidate` → `📋 Planned` flip). The architectural answer is the three-primitive design now committed in ARCHITECTURE §3.2.11 (Identity & conversation continuity): (a) a new `conversations` table holding the durable thread (`id`, `tenant_id`, `domain_id`, `created_at`, `last_activity_at`, `active`); (b) a nullable `conversation_id` FK on `sessions` pointing at `conversations.id` — session-linking, never session-merging, so the audit chain at the session granularity stays walkable; (c) a separate `identity_claims` table (`user_id`, `claim_type` ∈ {`email`, `phone`, `sso_subject`}, `claim_value`, `issuing_scope_id`, `issuing_adapter`, `verified_at` nullable, `active`) with case-folded uniqueness on (`claim_type`, `claim_value`, `issuing_scope_id`) so two scopes can independently assert the same number/email without colliding. The runtime surface is a `CrossSessionRetriever` in `app/memory/` (sibling to the existing per-session retriever) that, given the active session's `(conversation_id, tenant_id, domain_id)`, pulls the N most recent messages from sibling sessions under the same `conversation_id` and the same scope, ordered by recency, capped by a per-call budget. The auth surface stays unchanged: the existing embed-key + programmatic-API auth paths bind a request to a `(tenant, domain, agent)` scope first, then the cross-session retriever filters by that scope — three-layer scope enforcement (§4.7 of ARCHITECTURE) is preserved. The success criterion above is the v1 demonstration; closing tag will be `step-24-5c-cross-channel-identity-complete` cut on the doc-truthing commit of the impl arc (same shape as Step 30c's `99c6eb5` tag-on-doc-truthing pattern). Implementation backlog tracked at `DRIFTS.md` `D-step-24-5c-impl-backlog-2026-05-11`. Cross-refs: ARCHITECTURE §3.2.11 (new subsection landing in this same commit), ARCHITECTURE §3.3 step 5 (memory retrieval — cross-session retriever named as a sibling of the per-session retriever), ARCHITECTURE §4.9 (rejected alternative: session-merging — explicitly rejected to preserve the per-session audit boundary). Deliberate non-goals at v1, carried forward not as regressions: end-user-driven claim verification (deferred to Step 34a + Step 31 onboarding); cross-tenant identity federation (Step 38 territory by design — a claim is scoped to its issuing scope in v1); a per-user inbox view spanning all channels (Step 31 dashboards). |
...
| Hardening | **30c** | Action classification — tool invocations are tiered as routine, notify-and-proceed, or approval-required, so Luciel asks first only when an action is genuinely consequential. | Customers feel that Luciel acts decisively on routine work and pauses to confirm only when the stakes warrant it. An audit log can prove every approval-required action had a confirmation row preceding it. The behavior contract in Section 4 stops being aspirational and becomes enforced — with the right scope, not an annoying one. | ✅ Closed on 2026-05-11 (merged to `main` via four commits `b9d0520` → `becd92b` → `b216300` → `99c6eb5`, where the first three are the code-complete sequence and `99c6eb5` is the doc-truthing commit that aligns ARCHITECTURE, DRIFTS, and this row; closing tag `step-30c-action-classification-complete` is an annotated tag on `99c6eb5`, deliberately re-cut forward from the code-complete commit `b216300` onto the doc-truthing commit so the tag reads as "code + docs agree here"). The gate ships as a pluggable provider in `app/policy/action_classification.py` (parallel discipline to `app/policy/moderation.py` from Step 30d): `ActionTier` enum (ROUTINE / NOTIFY_AND_PROCEED / APPROVAL_REQUIRED), `StaticTierRegistryClassifier` reading each tool's `declared_tier` from the class body, `FailClosedActionClassifier` translating `ToolTierUndeclared` and any classifier exception into APPROVAL_REQUIRED, `NullActionClassifier` for dev with a per-call WARNING, and `ActionClassificationGate.from_settings` selecting the provider from two new `Settings` knobs (`action_classifier`, `action_classifier_fail_closed`) that default to the fail-closed static gate. `LucielTool.declared_tier` is the new class attribute every tool must override (the base class default is `None`, so a subclass that forgets a tier inherits APPROVAL_REQUIRED rather than silently executing). The three shipped tools were tiered in the same commit: `save_memory` and `get_session_summary` ROUTINE, `escalate_to_human` NOTIFY_AND_PROCEED. `ToolBroker.execute_tool` calls `self._classifier.classify(tool)` before `tool.execute(...)`, stamps `tier` / `tier_reason` / `classifier` onto every `ToolResult.metadata` with two distinct refusal codes so an audit log can tell failure modes apart: `tier_reason='unknown_tool'` when the broker cannot find the tool name in the registry (stray name / LLM hallucination), `tier_reason='tier_undeclared'` when a registered tool's maintainer forgot to declare a tier (the fail-closed wrapper catches `ToolTierUndeclared` and translates it), `tier_reason='classifier_error'` when the classifier raised any other exception, and `tier_reason='declared_tier'` on successful classification — all four refusal-class codes still route to APPROVAL_REQUIRED, the distinction is for the auditor, and short-circuits APPROVAL_REQUIRED with a pending frame containing `pending=True`, `tool_name`, and `proposed_parameters` so the runtime layer can render a confirmation prompt; the conversational shape of that prompt is the Runtime layer's job and is scoped with Step 31's richer widget UX. AST + behavioural test coverage in `tests/api/test_action_classification.py` (28 tests) pins the call-order invariant (`self._classifier.classify` before `tool.execute`), the APPROVAL_REQUIRED short-circuit branch returning before any execution, the three shipped tiers, the `LucielTool.declared_tier=None` default on the base class, every classifier provider's behaviour, fail-closed conversion of both `ToolTierUndeclared` and arbitrary exceptions, factory dispatch, and the broker integration end-to-end with a real `ToolRegistry`. A partner-level live end-to-end harness lives at `tests/e2e/step_30c_live_e2e.py` (33 claims, backend-free, runs in under a second) and exercises the shipped `ToolBroker` + `ToolRegistry` + shipped `LucielTool` subclasses against every English-language claim in this row — the first per-step live-e2e harness on the project, establishing a precedent documented in `tests/e2e/README.md` (one harness per closed step, asserts against this column's success criteria, a step is not "closed" until its harness is green on the closing branch). This harness caught an audit-signal collision (`tier_reason='unknown_tool'` was reused for two operationally-different failure modes) before merge that the 28 unit tests missed, and the surgical fix landed in the same branch. Closes `D-confirmation-gate-not-enforced-2026-05-09`. ARCHITECTURE §3.3 step 8 and §4.9 (synchronous-only rejected-alternative bullet) flipped 📋 → ✅ in the same commit. Deliberate non-goals at v1: real off-pattern detection (soft-dep on `D-context-assembler-thin-2026-05-09`), the customer-facing conversational confirmation UX (Step 31), and the structured per-invocation audit row write (broader audit work tracked in DRIFTS.md `D-widget-chat-no-application-level-audit-log-2026-05-10`). **Production enforcement first observed clean on 2026-05-11** via ECS rolling deploy: `luciel-backend-service` PRIMARY on `luciel-backend:39` and `luciel-worker-service` PRIMARY on `luciel-worker:19`, both pinned to ECR digest `sha256:f0bf303272fb0801eefc4cf0d20d2ddb624f2a5f60c8e845cbe422869739f863` built from `main` HEAD `84339a3`; backend logged the clean `Started server process [1]` → `Application startup complete.` → `Uvicorn running on http://0.0.0.0:8000` sequence followed by steady `GET /health → 200 OK`, worker logged `celery@...ready.` at 22:57:24 UTC with the old worker doing a clean `Warm shutdown` at 22:58:07 UTC, zero `ERROR`/`Traceback`/`CRITICAL` hits across the post-deploy log window, and the deploymentCircuitBreaker did not fire on either service. Manifest filenames in the repo renamed in the same doc-truthing commit (`td-backend-rev34.json` → `td-backend-rev39.json`, `td-worker-rev14.json` → `td-worker-rev19.json`) so the on-disk names match the registered task-def revisions. Runbook of record for this rollout: `docs/runbooks/step-30c-action-classification-deploy.md` (commit `401406d`). Full prod observed-clean stanza lives in DRIFTS §5 `~~D-confirmation-gate-not-enforced-2026-05-09~~`. One side-observation drift opened during the rollout (`D-celery-worker-runs-as-root-2026-05-11` — pre-existing root-uid worker container; not introduced by Step 30c). |
