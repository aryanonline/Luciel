...
| Frontend | **30b** | Embeddable chat widget that any company can drop into their existing website. | A company adds a few lines of code to their site, and within an hour their visitors are having real conversations with the company's Luciel. This is the unblock for the first paying customer. | 🔧 Build complete on `step-30b-chat-widget` (schema + endpoint + Preact bundle + e2e), `step-30b-embed-key-issuance` (POST /admin/embed-keys + scripts/mint_embed_key.py CLI), and `step-30b-widget-cdn-deploy` (CFN-provisioned S3 + CloudFront + GitHub OIDC deploy role + CI deploy job). First CI deploy uploaded the bundle and `https://d1t84i96t71fsi.cloudfront.net/widget.js` is reachable end-to-end (HTTP 200, application/javascript, 27300 bytes), but the deploy job exited red on a missing IAM permission for the post-upload wait step. `step-30b-cleanup` adds the IAM verb (`cloudfront:GetInvalidation`) and fixes a separate pre-existing CI red-window in the AST job; flips to ✅ once that branch lands a green end-to-end CI run on main and a real customer has exchanged messages through the published bundle. |
