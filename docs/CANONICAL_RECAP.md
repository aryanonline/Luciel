...
**Version:** v1.7
**Last updated:** 2026-05-05 ~10:35 EDT (Step 5 real-run first attempt FAILED CLEANLY on `pg_authid` privilege; advisor patched script + smoke test 11/11 GREEN; awaiting operator rebuild + td-rev3 + retry). After Commit `8d3ccb0` (Step 4 dry-run GREEN + recap v1.6), the Step 5 real-run on `luciel-mint:2` launched task `6dc293a3be784529948e7a7dc0e73091` cleanly, but the container exited 1 with `FATAL: role update failed: InsufficientPrivilege: permission denied for table pg_authid`. **Zero production state mutated** — the script's `verify_role_state()` query against `pg_authid` failed BEFORE `ALTER ROLE`; SSM `/luciel/production/worker_database_url` is still `ParameterNotFound`; `luciel_worker` password unchanged. Root cause: `pg_authid` is unreadable by `rds_superuser` on AWS RDS — only a true superuser can SELECT from it, and the RDS master role intentionally is not one. Resolution patch (uncommitted as of this banner): (1) `verify_role_state` now queries `pg_roles` (RDS-readable, sanitized view of `pg_authid`); (2) the lost rotation-vs-first-mint signal (`pg_authid.rolpassword IS NULL`, masked as `********` in `pg_roles`) is replaced with a stronger SSM-presence signal — new `verify_first_mint_or_force_rotate()` calls `ssm.get_parameter(/luciel/production/worker_database_url)`: `ParameterNotFound` ⇒ first mint, allow; parameter exists + `--force-rotate` ⇒ rotation, allow; parameter exists, no flag ⇒ refuse. New 11-test smoke suite at `tests/test_mint_worker_db_password_ssm.py` covers (a) SQL targets `pg_roles` not `pg_authid`, (b) SSM-presence guard for first-mint vs rotation, (c) env-only argv path uncontaminated by CLI flags (per `D-test-coverage-assumed-not-proven-…-2026-05-05` lesson). 11/11 GREEN locally. Two new drifts logged this version — see Section 15. Step 4 dry-run will need to be re-run on the new task-def revision (`luciel-mint:3`) before Step 5 retry. Previous version's bullet preserved verbatim below for audit continuity.
...
**Supersedes:** v1.6 (2026-05-05 ~10:19 EDT, Step 4 dry-run GREEN); v1.5 (2026-05-04 ~20:18 EDT, session-end pause pre-P3-S Half 2); v1.4 (2026-05-03 23:56 UTC, post P3-H); v1.3 (2026-05-03 late-evening, post P3-J/G/K); v1.2 (2026-05-03 evening, mid-Phase-2 docs reconciliation); v1.1 (2026-05-03 Phase 2 mid-stream close); v1 (2026-05-03 Phase 1 close)
