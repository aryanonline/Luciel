...
- [x] `docs/CANONICAL_RECAP.md` updated to v2.2 (v2.0 → v2.1 prod-verify 19/19 GREEN closure → v2.2 §8 read-only evidence sweep), tag `step-28-phase-2-complete` re-anchored at v2.2 commit
- [x] §8 read-only prod-hardening evidence captured (CW alarms inventoried + 1 misconfig fixed via Container Insights enable; ECS services 1/1 ACTIVE; autoscaling target confirmed 1–4; `pg_stat_activity` worker-role-split verified live at 2026-05-06T01:24:17 UTC — row `('luciel_worker', 'luciel', '', 'idle', 1)`). Evidence at `docs/evidence/step-28-phase-2/section-8-prod-hardening-evidence.md` + `docs/evidence/step-28-phase-2/pg_stat_activity-snapshot-2026-05-06-utc.log`. Three drifts logged this closure (D-worker-unhealthy-task-count-alarm-misconfigured-treatmissingdata-breaching-2026-05-05, D-worker-ecs-exec-not-enabled-pre-2026-05-05, D-pg-stat-activity-evidence-deferred-step29-2026-05-05); all three RESOLVED in-session per CANONICAL_RECAP §15
