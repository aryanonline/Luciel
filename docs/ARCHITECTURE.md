...
A prospect who chats with Luciel on a customer's website on Monday and calls the customer's Luciel-answered phone line on Wednesday should not have to re-introduce themselves. Continuity across channels is a product commitment (Recap §11 Q8, §13.1 T8), and the architecture answers it with three primitives that compose without breaking any existing scope or audit guarantee. 📋 Design committed in the Step 24.5c design-lock pass on 2026-05-11; implementation tracked at `DRIFTS.md` `D-step-24-5c-impl-backlog-2026-05-11`. The v1 implementation arc closes when two distinct channels (the embeddable chat widget and the programmatic API) exchange messages on two sessions joined by one `conversation_id` and one `identity_claims` row, with the cross-session retriever surfacing the sibling session's recent turns; the voice/SMS/email legs inherit the same primitives and become reachable end-to-end when Step 34a's channel adapter framework lands.
