...
**Triangulation:** Answers **Q5** (the self-serve sign-up / pay / change-plan / cancel surface for the Individual tier). Implements **Step 30a**. Closing tag: `step-30a-subscription-billing-complete`. Team / Company self-serve and annual / multi-SKU deferred to **Step 30a.1**. The password / SSO auth surface deferred to **Step 32**. Integrity questions: DRIFTS §3 `D-billing-team-company-not-self-serve-2026-05-13`, `D-magic-link-auth-cookie-session-2026-05-13`.
...
**Auth model — magic-link only at v1.** No password and no SSO at v1. The customer's email address — captured at Checkout and verified by Stripe's payment flow — is the identity. After `checkout.session.completed`, the webhook handler enqueues a magic-link email containing an HS256-signed JWT (`app/services/magic_link_service.py`, `[magic-link-email]` log marker on emit). The customer clicks the link, lands on `GET /api/v1/billing/login?token=…`, the backend exchanges the JWT for a 30-day signed session cookie, and the customer is redirected to `/account/billing`. The session cookie is the only credential the marketing site needs for `/portal` and `/me`. This is deliberately the minimum viable auth surface for the billing path; the broader auth story — password, SSO, multi-factor — lands at **Step 32** and is tracked at `D-magic-link-auth-cookie-session-2026-05-13`.
...
- Password / SSO / MFA auth. Magic-link is the v1 trust model. **Step 32** lands the broader auth story; the cookie format and the session lifetime chosen here are forward-compatible.
