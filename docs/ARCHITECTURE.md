...
- Password / SSO / MFA auth. Magic-link is the v1 trust model **through Step 30a.2-pilot**. **Step 30a.3** (re-scoped 2026-05-16, picking up the daily-login slice originally rolled into Step 32a, and sharpened 2026-05-16 by the partner's mandatory-at-signup decision — "password should be mandatory for all tiers, so they login whenever needed as long as their admin account is active") lands password authentication as the single daily-login primitive across every tier, with magic-link demoted to a recovery-and-invite-acceptance bootstrap. The implementation lands: a new `users.password_hash` column (argon2id at v1 over bcrypt for memory-hard resistance to GPU brute force, decided in the Step 30a.3 design pass), a `POST /api/v1/auth/login` route that mints the same `luciel_session` cookie the magic-link flow already mints, a `POST /api/v1/auth/set-password` route used at invite-acceptance, at first-time signup (the Stripe-Checkout post-success path lands here before the cookied redirect to `/app`), and at any later self-serve reset, and a `POST /api/v1/auth/forgot-password` flow that re-uses the existing magic-link mint to deliver a one-time reset link. **The Stripe Checkout success URL is rewritten in this step** — today the post-Checkout return URL is `/auth/return?token=<bootstrap_magic_token>` which mints the session cookie directly; post-Step-30a.3 it lands on `/auth/set-password?token=<bootstrap_magic_token>`, the user types a password, the form POSTs to `/api/v1/auth/set-password` (token-gated, no cookie required at this point), and only then does the session cookie mint and the redirect to `/app` fire. Magic-link is **not** a daily-login path for any tier post Step 30a.3; it is retained only for (a) the post-Checkout first-time password-set bootstrap (a magic-link-shaped token gates the `/auth/set-password` form so the email-controlling party is verifiably the buyer), (b) the invite-acceptance first-time password-set (same surface, different token mint path), and (c) the forgot-password reset. The cookie format and session lifetime chosen at Step 30a are forward-compatible with this expansion; Step 31.2's cookie middleware contract is unchanged — the cookie payload remains `{sub: user_id, scope: "session", iat, exp}` and the middleware does not inspect the auth path that minted it. SSO / MFA still land at **Step 32a** as the broader auth story. DRIFTS §3 `D-magic-link-only-auth-no-password-fallback-2026-05-16` carries the open gap until Step 30a.3 closes it.
