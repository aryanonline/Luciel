...
- Password / SSO / MFA auth. Magic-link is the v1 trust model **through Step 30a.2-pilot**. **Step 30a.3** (re-scoped 2026-05-16, picking up the daily-login slice originally rolled into Step 32a) lands password authentication with magic-link as the bootstrap-and-fallback primitive: a new `users.password_hash` column (bcrypt or argon2id at v1, decided in the Step 30a.3 design pass), a `POST /api/v1/auth/login` route that mints the same `luciel_session` cookie the magic-link flow already mints, a `POST /api/v1/auth/set-password` route used at invite-acceptance and at any later self-serve reset, and a `/forgot-password` flow that re-uses the existing magic-link mint to deliver a one-time reset link. Magic-link remains the bootstrap (signup, invite-acceptance, password reset); password becomes the daily-login primitive. The cookie format and session lifetime chosen at Step 30a are forward-compatible with this expansion; Step 31.2's cookie middleware contract is unchanged. SSO / MFA still land at **Step 32a** as the broader auth story. DRIFTS §3 `D-magic-link-only-auth-no-password-fallback-2026-05-16` carries the open gap until Step 30a.3 closes it.
