...
**Annotation 2026-05-20 (Step 30a.6 tier-hierarchy realignment).** The four-level scope hierarchy itself is unchanged at the data layer — `tenant → domain → agent → Luciel instance` remains the organizing principle, and every scope-bearing row keeps its foreign keys exactly as they are. What changes is the **billing-tier mapping onto that hierarchy**: a Team-tier tenant is now provisioned with `tenant → agent → Luciel instance` (no Domain layer minted at signup; `DOMAIN_COUNT_CAP_BY_TIER[TIER_TEAM]` flips `1 → 0` at Step 30a.6 Pass 3), while a Company-tier tenant keeps the full `tenant → domain → agent → Luciel instance` shape (multi-Domain remains the Company value driver). The Domain layer's data-model existence is unchanged — Team simply does not mint into it. The 1:1 "tier maps onto scope level" claim above is preserved for Individual (tenant scope, one user) and Company (tenant + Domains); Team's mapping is now "tenant scope with N agents directly under it", which still respects the no-leakage property because the Team's scope ceiling is the tenant. See CANONICAL_RECAP §11 Q1 annotation, §14 "Why the price difference is what it is" rewrite, and DRIFTS §3 `D-tier-semantics-realignment-2026-05-20`.
