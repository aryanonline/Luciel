...
**Tenancy model note (2026-05-22, Arc 4):** The canonical tenancy model evolved on 2026-05-22 from `Tenant → Domain → Agent → Lead` (four levels, tier-shape-branched) to `Admin → Instance → Lead` (three levels, uniform across all tiers). The Domain layer is removed entirely; `Tenant` is renamed `Admin`; `Agent` is renamed `Instance`. Every subsection below that names these nouns reflects the pre-collapse shape in its existing prose; the doc-level annotation here is the forward reference. The execution arc (schema migration, code rename, Stripe restructure) is tracked at CANONICAL_RECAP §14 and DRIFTS §3 `D-tenancy-collapse-admin-instance-lead-2026-05-22`.
...
**Tenancy model update (2026-05-22, Arc 4):** Under the Admin → Instance → Lead model, the dashboard hierarchy evolves to **Admin rollup / Instance-group view / Single-instance view**. The three-method dashboard-service shape is preserved at the FK layer (method signatures rename from `tenant_id`/`domain_id`/`agent_id` to `admin_id`/`instance_group_id`/`instance_id`). Role-based filtering replaces the tenant/domain/agent FK chain as the scope-selection mechanism: an `owner` sees the Admin rollup; an `instance_lead` sees the Instance-group pane for their assigned Instances; a `member` sees only their assigned Single-instance detail. The execution arc for this rename is tracked at CANONICAL_RECAP §11 Q2 and DRIFTS §3 `D-tenancy-collapse-admin-instance-lead-2026-05-22`.
...
**Tenancy model update (2026-05-22, Arc 4):** Under the Arc 4 collapse, the billing entity is `Admin` (renamed from `Tenant`); the `subscriptions.tenant_id` FK renames to `subscriptions.admin_id`. Stripe `customer.subscription.*` webhooks map 1:1 from old to new — this is a mechanical rename at the data layer. The existing cancel cascade, refund cascade, and retention purge worker are unaffected at the runtime layer; only the column name and service noun change. The Stripe SKU restructure (renaming Individual→Solo in product metadata) is a separate execution-arc step tracked at DRIFTS §3 `D-tenancy-collapse-admin-instance-lead-2026-05-22`.
...
**Annotation 2026-05-20 (Step 30a.6 tier-hierarchy realignment).** The four-level scope hierarchy itself was unchanged at the data layer at that step — `tenant → domain → agent → Luciel instance` remained the organizing principle; Team was flattened (`tenant → agent`, no Domain layer minted at signup) while Company kept the full three-level shape. See DRIFTS §3 `D-tier-semantics-realignment-2026-05-20`.

**Annotation 2026-05-22 (Arc 4 tenancy collapse).** The scope hierarchy is now `Admin → Instance(s) → Lead(s)` — uniform across every tier. The Domain layer is removed entirely. The billing boundary is the `Admin` row (renamed from `Tenant`); one Stripe subscription per Admin. The tier no longer determines topology; it determines capacity gates along the six entitlement axes (CANONICAL_RECAP §14). The 1:1 claim — "tier maps onto scope level" — is superseded by "tier maps onto gate settings over a single uniform topology." The FK chain, the DB-grant enforcement, and the request-scope middleware all rebind from `tenant_id`/`domain_id`/`agent_id` to `admin_id`/`instance_id` — the three-layer enforcement discipline is preserved, only the axis names change. The execution arc (schema migration, code rename) is tracked at DRIFTS §3 `D-tenancy-collapse-admin-instance-lead-2026-05-22`. See CANONICAL_RECAP §11 Q1 and §14 for the business view.
...
**Annotation 2026-05-20 (Step 30a.6 entitlement-matrix extension).** The three-layer scope-enforcement model above governs **scope-boundary crossings** (an agent-scope caller cannot read another agent's data). It does not govern **tier-entitlement enforcement** (a Team-tier caller cannot invoke a Company-only capability like cross-Domain memory, audit CSV export, custom widget branding, or SSO). Step 30a.6 lands `app/policy/entitlements.py` as the policy-module home for tier-entitlement enforcement, sibling to the scope-enforcement model in this section, not a replacement for it. The two policies compose at every request: scope is checked first (Layer 1 + Layer 2 + Layer 3 above), then entitlement is checked at the service layer against the caller's tenant's tier. The same fail-closed posture applies — a missing entitlement entry inherits the most-restrictive value (no access) rather than silently allowing the call. Cheap entitlement checks (seats, leads cap, domains cap, instance cap) wire to existing call-sites today; the eight Roadmap rows in CANONICAL_RECAP §14's entitlement matrix are deferred under `D-entitlement-matrix-v1-roadmap-rows-deferred-2026-05-20` and land at their corresponding follow-up Steps. See CANONICAL_RECAP §14 "Entitlement matrix" sub-section.

**Annotation 2026-05-22 (Arc 4 tenancy collapse).** The three-layer scope-enforcement model is unchanged in structure; only the axis names change. The three layers now enforce `admin` / `instance` / `lead` boundaries (renamed from `tenant` / `domain` / `agent`). The entitlement enforcement from Step 30a.6 (gated by `app/policy/entitlements.py`) is preserved as a sibling check. See CANONICAL_RECAP §11 Q1 and DRIFTS §3 `D-tenancy-collapse-admin-instance-lead-2026-05-22`.
