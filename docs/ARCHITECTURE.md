...
**Paid-intro trial mechanics (Step 30a.2, 2026-05-14, supersedes the mixed per-tier trial policy above).** The six recurring SKUs are unchanged; what changed is the trial shape that wraps them. A first-time customer pays a one-time CAD $100 intro fee at Checkout, then receives a uniform 90-day trial on the recurring SKU of any of the six (tier, cadence) pairs. The trial converts to the recurring rate on day 91 unless the customer cancels. Three implementation primitives carry this: (a) a seventh `stripe_price_intro_fee` Price (one-time, CAD $100, configured at the SSM key `/luciel/prod/stripe/price_id/intro_fee` alongside the six recurring keys); (b) a `BillingService.is_first_time_customer(email)` predicate that performs a case-insensitive lookup on the `subscriptions.customer_email` column via `func.lower(Subscription.customer_email) == func.lower(email)` — once a row exists for an email under any tier or cadence, that email has been a customer and the gate flips closed for life (cancel-and-rejoin returns to the standard recurring rate with no intro fee and no fresh trial); (c) Stripe Option A line-item assembly in `BillingService.create_checkout_session` — first-time signups get a two-line-item Checkout session (the one-time intro Price + the recurring Price with `subscription_data.trial_period_days=90`), and rejoining customers get a one-line-item session (the recurring Price, no trial, no intro Price). The `INTRO_TRIAL_DAYS=90` constant replaces the per-tier `TRIAL_DAYS` map; `payment_method_collection='always'` is retained from Step 30a so the card is captured at signup even during the trial window. The audit row for `SUBSCRIPTION_CREATED` carries a `first_time_customer: bool` field so the operator dashboard can answer "how many of this week's new subscribers got the intro offer vs. were returning customers" without re-querying Stripe.
...
**Team-invite path (Step 30a.1 commit G).** The `/admin/luciel-instances` POST handler accepts an optional `teammate_email` field. When present (and the calling tenant's tier is `team` or `company`), the handler skips the normal `LucielInstance` creation flow and instead mints a `User` row for the teammate, an `Agent` row representing the teammate as a principal, a `ScopeAssignment` row binding the teammate to the calling tenant's domain (for `team`) or tenant (for `company`) scope, and an `EmailService.send_magic_link` email so the teammate can sign in. The teammate's first login lands them on `/dashboard` as a domain- or tenant-scoped member; their view of the Luciels tab is filtered by their scope assignment per the same three-layer scope enforcement §4.7 applies everywhere. No new Stripe charge is created — the teammate is a member of the existing tenant's subscription, not a separately-billable seat. (The tier-as-product model rejects per-seat metering; CANONICAL_RECAP §14.)
...
- Password / SSO / MFA auth. Magic-link is the v1 trust model. **Step 32a** (re-targeted from Step 32 on 2026-05-13 when Step 32's scope locked to admin dashboard UI) lands the broader auth story; the cookie format and the session lifetime chosen here are forward-compatible. Step 31.2 (2026-05-13) extended the same cookie format to authenticate `/admin` and `/dashboard` routes without changing the cookie itself — a future password / SSO authenticator landing at Step 32a will issue the same cookie shape, so neither this subsection nor the middleware contract needs to change at that step.
