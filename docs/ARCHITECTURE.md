...
**Last updated:** 2026-05-11 (Step 30d Deliverable C closeout: no new architectural prose added — the widget-surface E2E CI harness is build-time detail per DRIFTS §6 doctrine ("build-time detail belongs in commit messages, not canonical docs; canonical docs grow only when architectural shape changes"); the §3.3 step 6.5 and §4.9 entries from Deliverable B continue to describe the moderation-gate design correctly, and the §3.2.2 *Issuance* paragraph from Deliverable A continues to describe the preflight validator correctly; harness mechanics live in commit messages on `step-30d-e2e-ci-gate` (commits `7d287ca` / `2e545b4` / `9b7663f`) and in `ci/e2e/README.md`. Prior 2026-05-11 update: §3.3 gains new step 6.5 documenting the widget-surface content-safety moderation gate added in Step 30d Deliverable B; §4.9 gains a new rejected-alternative bullet on prompt-only content gating. Prior 2026-05-11 update: §3.2.2 *Issuance* paragraph extended with one sentence documenting the scope-prompt preflight added in Step 30d Deliverable A; tenant-wide mints skip the preflight and surface a non-fatal warning. Earlier 2026-05-11 update: new §3.2.2 documented the widget CDN tier and the embed-key public-surface model; old §3.2.2 – §3.2.9 each shifted by one; §3.2.1 updated to reflect chat-widget live; §3.2.7 Application log stream bullet renumbered from §3.2.6 and retains its 📋 mark for the widget-chat audit gap.)
