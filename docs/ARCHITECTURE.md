...
**Triangulation:** Answers **Q5** (the self-serve sign-up / pay / change-plan / cancel surface for the Individual tier). Implements **Step 30a** (billing surface), **Step 31.2** (cookie→admin-context bridge — extends the same session cookie to `/admin` and `/dashboard`), **Step 32** (admin dashboard UI — the customer-facing surface that consumes both the billing routes and the bridged admin routes from `aryanonline/Luciel-Website` at `/dashboard`, `/dashboard/luciels/:pk`, and `/account/billing`). Closing tags: `step-30a-subscription-billing-complete`, `step-31-2-cookie-bridge-and-instance-embed-keys-complete`, `step-32-admin-dashboard-ui-complete`. Team / Company self-serve and annual / multi-SKU deferred to **Step 30a.1**. The password / SSO auth surface deferred to **Step 32a** (re-targeted from Step 32 on 2026-05-13 when Step 32's scope locked to admin dashboard UI). Integrity questions: DRIFTS §3 `D-billing-team-company-not-self-serve-2026-05-13`, `D-magic-link-auth-cookie-session-2026-05-13`, `D-admin-audit-logs-actor-user-id-fk-missing-2026-05-13`.
...
**`subscriptions` table — one row per Stripe subscription bound to one Luciel tenant.** A new table (`app/models/subscription.py`, Alembic migration `b8e74a3c1d52`, down_revision `3dbbc70d0105`) holds one row per Stripe subscription with FK back to `tenants.id`. The row carries the Stripe identifiers (`stripe_customer_id`, `stripe_subscription_id`, `stripe_price_id`), the tier (`individual` at v1, `team` / `company` reserved for Step 30a.1), the lifecycle state (`status` enum mirroring Stripe's `trialing` / `active` / `past_due` / `canceled` / `incomplete` / `unpaid` plus a derived `is_entitled` predicate), the period-window fields (`current_period_start`, `current_period_end`, `trial_end`, `cancel_at_period_end`, `canceled_at`), the customer email captured at Checkout time, and `last_event_id` — the idempotency cursor against Stripe's `event.id`. Scope-bearing per §4.1 (every row has a foreign key to its tenant; the three-layer enforcement in §4.7 applies the same way it does to every other scope-bearing row). A subscription belongs to exactly one tenant; one tenant has at most one active subscription at v1 (multi-product-per-tenant is Step 30a.1 territory).

**Seven-route HTTP surface (`app/api/v1/billing.py`).**
- `POST /checkout` — public; the marketing site posts `{email, display_name, tier='individual', tenant_id?}` and receives a Stripe Checkout session URL. Creates the Stripe session in `mode=subscription`, attaches the configured CAD price, opts into Stripe Tax, and sets `subscription_data.trial_period_days=14` plus `payment_method_collection='always'` (card required even during trial).
...
**Currency, tax, and pricing — CAD-only at v1.** All amounts are in cents at the Stripe boundary (Stripe's wire format). The single Individual SKU is locked at $30 CAD/month with a 14-day free trial (card required during trial). Stripe Tax is enabled so the customer sees the right line items at Checkout for their billing address; we do not maintain a tax table ourselves. Annual pricing, multi-SKU, and Team / Company self-serve are explicitly out of scope at v1 — tracked at `D-billing-team-company-not-self-serve-2026-05-13`, naturally landing at **Step 30a.1**.

**What this design deliberately does not solve in v1** (carried forward, not regressions):
- Team / Company self-serve. Both tiers stay waitlist-only on the marketing site; the Pricing CTAs for them route through the existing waitlist modal, not through `/signup`. **Step 30a.1** lands self-serve for those tiers alongside annual pricing and the multi-SKU shape.
