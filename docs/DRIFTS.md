...
**Status:** OPEN — process drift; needs a roadmap home (CI gate, not a code commit)
...
**Owning roadmap step:** None today. Recommend adding a sub-deliverable to **Step 30b's tail** or as a new **Step 30b-CI** line: a containerized end-to-end gate that, on every PR touching `app/api/v1/chat_widget.py`, `app/api/v1/admin.py` (embed-keys handler), `app/api/widget_deps.py`, or `app/middleware/auth.py`, (a) brings up backend + Postgres + Redis, (b) seeds a tenant + domain + embed key, (c) POSTs a widget message, (d) asserts HTTP 200, `text/event-stream` content-type, and a non-empty first SSE frame. Without this, every widget-surface PR is one production deploy away from a fourth bug of the same class.
**Doc-truthing this pass:** No CANONICAL_RECAP or ARCHITECTURE prose change. The Step 30b row in §12 stays 🔧 until the CI gate is in place and the staging widget passes end-to-end (technical green + safety green per the drift below). This drift closes when the CI gate ships and a widget-surface PR is gated by it.

### D-widget-no-content-safety-or-scope-guardrail-2026-05-10

**Status:** OPEN — load-bearing for customer trust; needs a roadmap home
...
**Owning roadmap step:** None today. Recommend a new **Step 30d (Widget content safety and scope guardrails)** sitting between Step 30c (action classification — tool gating) and Step 31 (dashboards + validation gate). Step 30c gates *what Luciel does*; Step 30d gates *what Luciel says and what it engages with*. They are sibling problems, not parent/child, and folding 30d into 30c would dilute both. Step 30d's success criterion: with a real REMAX-style domain config, the widget (1) declines an off-topic question with a graceful redirect to the customer's vertical and (2) refuses sensitive-content engagement with a fixed, sanitized response that never reaches the LLM provider's traces in raw form.
...
**Doc-truthing this pass:** No CANONICAL_RECAP or ARCHITECTURE prose change yet (Step 30d is not yet committed to as a roadmap line). When the operator approves Step 30d, this drift's resolution path links to that step and CANONICAL_RECAP §12 receives the new row. Until then, the drift carries the design claim explicitly.
...
**Design claim:** CANONICAL_RECAP §13 (production scenarios) implies per-tenant observability for any customer-facing surface — "we can answer is Luciel earning its keep here in under a minute." ARCHITECTURE §3.5 (telemetry and audit logs) states that every chat turn emits a structured record sufficient to attribute usage, debug failures, and feed Step 31 dashboards. Neither matches what `/ecs/luciel-backend` currently shows for the widget surface.
...
**Doc-truthing this pass:** Step 30b row in CANONICAL_RECAP §12 updated to record stage-1 (Luciel-owned staging E2E) observed clean on 2026-05-10, with the three bugs found-and-fixed and the three open drifts (this one, route-coverage, content-safety) called out by id. Marker stays 🔧 — the row's literal success criterion is REMAX-customer visitors exchanging real conversations, which has not happened yet. ARCHITECTURE §3.5 receives a 📋 marker on the audit-log paragraph; the prose stays unchanged because the design is correct, the implementation is incomplete.
