...
**Status:** OPEN [PROD-PHASE-2B] — reachable end-to-end on first deploy, IAM polish pending
...
**Progress on `step-30b-widget-cdn-deploy` (merged 2026-05-10 as `85990c4`):** All four resolution-path items shipped:
...
**Why this drift is still open:** The first CI run failed at phase 5 (`aws cloudfront wait invalidation-completed`) with `AccessDenied: cloudfront:GetInvalidation`. The CFN deploy role's inline policy granted `CreateInvalidation` and `GetDistribution` but not `GetInvalidation`, so the wait step could not poll. The deploy itself succeeded (the invalidation that the wait step was polling completed normally a few seconds later); only the deploy-job exit code went red. Closing this drift requires the next CI run to go green end-to-end after `step-30b-cleanup` lands the IAM fix (`cloudfront:GetInvalidation` added to the role, see commit `ef7cbf7` on this branch) and the operator runs `aws cloudformation deploy --stack-name luciel-widget-cdn ...` to apply the in-place IAM update. After the next merge-to-main triggers a green CI run — both jobs passing — the closing tag `step-30b-widget-cdn-complete` lands on that merge commit and this drift moves to Section 5.
