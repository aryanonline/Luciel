...
**Status:** RESOLVED on 2026-05-11 (build complete on `step-30c`, merged to `main`, closing tag cut).
...
**Cross-refs:** ARCHITECTURE §3.3 step 8 (design statement, now ✅; also names the four `tier_reason` codes); ARCHITECTURE §4.9 (synchronous-only rejected-alternative bullet, now ✅; also names the live-e2e harness); CANONICAL_RECAP §4 ("On consequential" note, now ✅); CANONICAL_RECAP §5 step 4 (operating loop, now ✅); CANONICAL_RECAP §12 Step 30c row (now ✅; names all four `tier_reason` codes and the live-e2e harness); `tests/api/test_action_classification.py` (28 unit tests); `tests/e2e/step_30c_live_e2e.py` (33-claim live e2e harness); `tests/e2e/README.md` (the per-step live-e2e harness precedent); soft-dep on `D-context-assembler-thin-2026-05-09` (off-pattern detector); downstream dep `D-widget-chat-no-application-level-audit-log-2026-05-10` (per-invocation audit row).
