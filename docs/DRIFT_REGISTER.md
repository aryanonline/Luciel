...
| `D-pillar13-spoof-audit-poll-too-short-2026-05-07` | C15 _this commit_ | closed | P13 (cross-tenant identity) | P13 polled for the `ACTION_WORKER_IDENTITY_SPOOF_REJECT` audit row with a 60s sleep + single 5s retry (65s total). Under `--pool=solo` worker contention (P12 = 42s legit extraction, P13 setup turn = ~10s legit extraction queued before the spoof), the spoof task sits behind those tasks and Gate 6's audit row lands 70-80s after enqueue. The route, the worker, and Gate 6 are all correct — verified directly: `admin_audit_logs` row id=6224 with action `worker_identity_spoof_reject`, correct `actor_key_prefix`, correct mismatch note, `created_at` 2026-05-08 04:06:21+00. Test bug only. Fix: replace single retry with a deterministic poll loop mirroring Pillar 25's contract (12 attempts x 5s = 60s extra, total ~120s hard cap). Production behavior unchanged |
