...
| `D-celery-app-not-imported-on-uvicorn-boot-2026-05-07` | C13 (`0922cdf`) | closed | P25 (worker pipeline liveness) + P13 (cross-tenant identity) / latent prod | `app/main.py` did not import `app.worker.celery_app`, so `@shared_task` on `extract_memory_from_turn` (and any other task) bound to Celery's default `current_app` whose default broker is `amqp://guest@localhost//`. Pillar 25 probe and any chat-turn async enqueue on a fresh uvicorn process would raise `kombu.exceptions.OperationalError` (port 5672 refused) instead of publishing to the configured Redis/SQS broker. P13 also depended on this path: its spoof payload is enqueued via `extract_memory_from_turn.delay(...)` from inside the verification harness running in the uvicorn process, so the spoof never reached the worker and Gate 6 never emitted `ACTION_WORKER_IDENTITY_SPOOF_REJECT`. Fix: import `celery_app` in `app/main.py` (canonical) and `app/memory/service.py` (defense in depth). Latent prod risk because verification harness imports `_infra_probes.py` which loads `celery_app` and masked the bug in dev — first real production chat turn after a clean uvicorn boot would have hit this |
| `D-pillar14-consent-grant-uses-wrong-tenant-key-2026-05-07` | C14 _this commit_ | closed | P14 (departure semantics) | `pillar_14_departure_semantics.py` issued the T2 consent grant with `k1_raw` (T1's chat key) but specified `tenant_id=t2_id` in the body. `consent.py:_resolve_tenant_id` correctly rejected this as cross-tenant (`403 cross_tenant_denied`), surfacing as a P14 FAIL. Test bug, not a route bug — the production contract is correct and stays untouched. Fix: swap `k1_raw` -> `k2_raw` so the T2 consent is issued with T2's own key, matching the symmetric K1/T1 consent grant 27 lines earlier |
