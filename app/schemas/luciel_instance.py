...
class LucielInstanceCreate(BaseModel):
    ...
    # ---------------------------------------------------------------
    # Step 30a.5 — teammate_email removed; use POST /admin/invites
    # ---------------------------------------------------------------
    #
    # The Step 30a.1 ``teammate_email`` invite-mode overload was
    # removed in Step 30a.5. The first-class invite path is POST
    # /admin/invites (Step 30a.4) -- it mints the User + Agent +
    # ScopeAssignment + UserInvite + welcome email atomically, and
    # carries a ``role`` field so the inviter picks tenant_admin /
    # department_lead / teammate explicitly. See
    # docs/designs/step-30a-5-company-self-serve.md §8 for the
    # removal rationale.
