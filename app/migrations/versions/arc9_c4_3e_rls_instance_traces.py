"""Arc 9 C4.3e -- Wall 3 RLS policy on traces (luciel_instance_id).
...
materialises Wall 3's Layer 2 (PostgreSQL RLS) on traces.
...
revision = "arc9_c4_3e_rls_instance_traces"
down_revision = "arc9_c4_3d_rls_instance_sessions"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE POLICY traces_instance_isolation
        ON traces
        AS PERMISSIVE
        FOR ALL
        TO PUBLIC
