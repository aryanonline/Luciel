...
def worker_pipeline_probe_step29y(
    ...
    # ---- Step 29.y diag: dump in-process producer/connection state ----
    # Surfaces the live uvicorn process's view of celery config at the
    # exact moment of publish, so we can compare against a fresh-process
    # baseline (which works). Wrapped to never affect the route's behavior.
    try:
        ...
        _bto = _capp_diag.conf.broker_transport_options
        _diag_logger.error(
            "STEP29Y_DIAG conf.broker_transport_options=%s",
            _json_diag.dumps(_bto, default=str),
        )
        _diag_logger.error(
            "STEP29Y_DIAG conf.broker_url=%s",
            str(_capp_diag.conf.broker_url)[:80],
        )
        try:
            _amqp = _capp_diag.amqp
            _pp_conn = _amqp.producer_pool.connections.connection
            _diag_logger.error(
                "STEP29Y_DIAG pp_conn.transport_options=%s",
                _json_diag.dumps(getattr(_pp_conn, "transport_options", {}), default=str),
            )
            _diag_logger.error(
                "STEP29Y_DIAG pp_conn.as_uri=%s",
                str(_pp_conn.as_uri())[:80],
            )
        except Exception as _e_pp:
            _diag_logger.error("STEP29Y_DIAG pp_inspect_error=%r", _e_pp)
        try:
            _conn_w = _capp_diag.connection_for_write()
            _diag_logger.error(
                "STEP29Y_DIAG conn_for_write.transport_options=%s",
                _json_diag.dumps(getattr(_conn_w, "transport_options", {}), default=str),
            )
        except Exception as _e_cw:
            _diag_logger.error("STEP29Y_DIAG conn_for_write_error=%r", _e_cw)
        _diag_logger.error(
            "STEP29Y_DIAG thread.name=%s thread.ident=%s",
        ...
        _log_diag2.getLogger("luciel.step29y.diag").error(
            "STEP29Y_DIAG outer_error=%r", _diag_err
        )
    # ---- end Step 29.y diag ----
