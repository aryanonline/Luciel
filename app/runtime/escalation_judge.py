...
# The score fires when it reaches this threshold (any single scored
# signal is >= 0.3; budget alone = 0.5; intent alone = 0.4).
LEAD_SCORE_THRESHOLD: float = 0.3

# LEGACY COMPAT: FREE_LEAD_BUDGET_THRESHOLD is kept so existing tests
# that import it do not break. It is no longer used for the scoring
# decision (replaced by LEAD_SCORE_THRESHOLD on the normalized score),
# but its presence here documents the old drift and allows the tests that
# referred to it to be updated in lockstep.
# RESCAN TIER-C: this constant is intentionally retained for backward-
# compatibility with tests that imported it. The scoring logic has moved
# from "lead_value >= 750_000" to "lead_score >= LEAD_SCORE_THRESHOLD".
