"""
Arc 9 C6.3 -- Mint the luciel_ops Postgres password and write the
ops DB connection string to SSM as SecureString.

Derived from scripts/mint_worker_db_password_ssm.py (Step 28 Commit 8).
Identical Pattern E + Pattern N discipline -- the password's only
plaintext home is SSM (KMS-encrypted), never stdout/CloudWatch/shell
history/git. The only structural difference is the target role
(luciel_ops vs luciel_worker), the SSM path, and the error-message
role-reference text.

The luciel_ops role (created by Alembic migration
arc9_c6_1_luciel_ops_role) has BYPASSRLS + narrow grants -- see that
migration's docstring for the full role-design rationale.

Original worker-mint header preserved below for cross-reference:
...
docs/runbooks/step-28-commit-8-luciel-worker-sg.md):
    ...
    python -m scripts.mint_ops_db_password_ssm \\
        ...
        --ssm-path     "/luciel/production/ops_database_url" \\
        --region       "ca-central-1"
...
# Renamed from WORKER_ROLE_NAME -- kept the variable name for minimal
# diff against the worker-mint template. This script mints luciel_ops.
WORKER_ROLE_NAME = "luciel_ops"
DEFAULT_SSM_PATH = "/luciel/production/ops_database_url"
...
def verify_role_state(conn) -> None:
    ...
    if row is None:
        raise RuntimeError(
            ...
            f"migration arc9_c6_1_luciel_ops_role before running this "
            f"script."
        )
...
def write_ssm(
    *,
    ...
    ssm.put_parameter(
        ...
        Overwrite=True,
        Description=(
            f"Ops DB connection string for {WORKER_ROLE_NAME} role "
            f"(BYPASSRLS, narrow grants for retention/deletion sweeps). "
            f"Minted via scripts.mint_ops_db_password_ssm (Arc 9 C6.3)."
        ),
    )
