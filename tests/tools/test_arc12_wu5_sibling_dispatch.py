...
def _restore_prod_audit_chain_handler():
    ...
    process, so a later real-Postgres test writes zero-hash audit rows
    that collide on ``ux_admin_audit_logs_row_hash``. This fixture strips
    the stub and reinstalls the prod handler once the test finishes.
    """
    yield
