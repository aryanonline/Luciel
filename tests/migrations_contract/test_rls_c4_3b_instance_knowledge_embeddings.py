"""
Arc 9 C4.3b regression tests -- shape of the Wall-3 RLS policy on knowledge_embeddings.
...
Asserts (via regex over the migration source, not against a live DB):
  ...
  4. Upgrade ENABLEs RLS on knowledge_embeddings.
  5. Upgrade CREATEs POLICY knowledge_embeddings_instance_isolation -- name MUST
...
class TestC43BMigrationShape(unittest.TestCase):
    ...
    def test_upgrade_enables_rls(self):
        self.assertRegex(
            self.src,
            r"ALTER\s+TABLE\s+knowledge_embeddings\s+ENABLE\s+ROW\s+LEVEL\s+SECURITY",
        )
    ...
    def test_upgrade_creates_instance_policy(self):
        ...
        self.assertRegex(
            self.src,
            r"CREATE\s+POLICY\s+knowledge_embeddings_instance_isolation",
        )
        # Make sure we did NOT accidentally reuse the Wall-1 name.
        self.assertNotRegex(
            self.src,
            r"CREATE\s+POLICY\s+knowledge_embeddings_tenant_isolation",
        )
    ...
    def test_downgrade_drops_policy(self):
        self.assertRegex(
            self.src,
            r"DROP\s+POLICY\s+IF\s+EXISTS\s+knowledge_embeddings_instance_isolation",
        )

    def test_downgrade_keeps_rls(self):
        disable_pattern = (
            r"ALTER\s+TABLE\s+knowledge_embeddings\s+DISABLE\s+ROW\s+LEVEL\s+SECURITY"
        )
...
class TestC43BMigrationImports(unittest.TestCase):
    def test_module_imports_cleanly(self):
        from alembic.script import ScriptDirectory
        from alembic.config import Config
        cfg = Config(str(ALEMBIC_INI))
        sd = ScriptDirectory.from_config(cfg)
        rev = sd.get_revision("arc9_c4_3b_rls_instance_knowledge_embeddings")
        self.assertIsNotNone(rev)
        self.assertEqual(rev.down_revision, "arc9_c4_3a_rls_instance_api_keys")
