"""
Arc 9 C4.3e regression tests -- shape of the Wall-3 RLS policy on traces.
...
Asserts (via regex over the migration source, not against a live DB):
  ...
  4. Upgrade ENABLEs RLS on traces.
  5. Upgrade CREATEs POLICY traces_instance_isolation -- name MUST
...
MIGRATION_PATH = Path(__file__).resolve().parents[2] / "alembic/versions/arc9_c4_3e_rls_instance_traces.py"


class TestC43EMigrationShape(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_upgrade_enables_rls(self):
        self.assertRegex(
            self.src,
            r"ALTER\s+TABLE\s+traces\s+ENABLE\s+ROW\s+LEVEL\s+SECURITY",
        )
    ...
    def test_upgrade_creates_instance_policy(self):
        ...
        self.assertRegex(
            self.src,
            r"CREATE\s+POLICY\s+traces_instance_isolation",
        )
        # Make sure we did NOT accidentally reuse the Wall-1 name.
        self.assertNotRegex(
            self.src,
            r"CREATE\s+POLICY\s+traces_tenant_isolation",
        )
...
class TestC43EMigrationImports(unittest.TestCase):
    def test_module_imports_cleanly(self):
        ...
        rev = sd.get_revision("arc9_c4_3e_rls_instance_traces")
        self.assertIsNotNone(rev)
        self.assertEqual(rev.down_revision, "arc9_c4_3d_rls_instance_sessions")
