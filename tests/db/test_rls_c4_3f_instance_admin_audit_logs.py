"""
Arc 9 C4.3f regression tests -- shape of the Wall-3 RLS policy on admin_audit_logs.
...
Asserts (via regex over the migration source, not against a live DB):
  ...
  4. Upgrade ENABLEs RLS on admin_audit_logs.
  5. Upgrade CREATEs POLICY admin_audit_logs_instance_isolation -- name MUST
...
MIGRATION_PATH = Path(__file__).resolve().parents[2] / "alembic/versions/arc9_c4_3f_rls_instance_admin_audit_logs.py"


class TestC43FMigrationShape(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_revision_id(self):
        ...
        self.assertEqual(m.group(1), "arc9_c4_3f_rls_instance_admin_audit_logs")

    def test_down_revision(self):
        m = re.search(r'^down_revision\s*=\s*"([^"]+)"', self.src, re.M)
        self.assertIsNotNone(m)
        self.assertEqual(m.group(1), "arc9_c4_3e_rls_instance_traces")

    def test_upgrade_enables_rls(self):
        self.assertRegex(
            self.src,
            r"ALTER\s+TABLE\s+admin_audit_logs\s+ENABLE\s+ROW\s+LEVEL\s+SECURITY",
        )
    ...
    def test_upgrade_creates_instance_policy(self):
        ...
        self.assertRegex(
            self.src,
            r"CREATE\s+POLICY\s+admin_audit_logs_instance_isolation",
        )
        # Make sure we did NOT accidentally reuse the Wall-1 name.
        self.assertNotRegex(
            self.src,
            r"CREATE\s+POLICY\s+admin_audit_logs_tenant_isolation",
        )
    ...
    def test_downgrade_drops_policy(self):
        self.assertRegex(
            self.src,
            r"DROP\s+POLICY\s+IF\s+EXISTS\s+admin_audit_logs_instance_isolation",
        )

    def test_downgrade_keeps_rls(self):
        disable_pattern = (
            r"ALTER\s+TABLE\s+admin_audit_logs\s+DISABLE\s+ROW\s+LEVEL\s+SECURITY"
        )
...
class TestC43FMigrationImports(unittest.TestCase):
    def test_module_imports_cleanly(self):
        from alembic.script import ScriptDirectory
        from alembic.config import Config
        cfg = Config("/home/user/workspace/luciel/alembic.ini")
        sd = ScriptDirectory.from_config(cfg)
        rev = sd.get_revision("arc9_c4_3f_rls_instance_admin_audit_logs")
        self.assertIsNotNone(rev)
        self.assertEqual(rev.down_revision, "arc9_c4_3e_rls_instance_traces")
