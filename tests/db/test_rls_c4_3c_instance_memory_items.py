"""
Arc 9 C4.3c regression tests -- shape of the Wall-3 RLS policy on memory_items.
...
Asserts (via regex over the migration source, not against a live DB):
  ...
  4. Upgrade ENABLEs RLS on memory_items.
  5. Upgrade CREATEs POLICY memory_items_instance_isolation -- name MUST
...
MIGRATION_PATH = Path(__file__).resolve().parents[2] / "alembic/versions/arc9_c4_3c_rls_instance_memory_items.py"


class TestC43CMigrationShape(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_down_revision(self):
        ...
        self.assertIsNotNone(m)
        self.assertEqual(m.group(1), "arc9_c4_3b_rls_instance_knowledge_embeddings")

    def test_upgrade_enables_rls(self):
        self.assertRegex(
            self.src,
            r"ALTER\s+TABLE\s+memory_items\s+ENABLE\s+ROW\s+LEVEL\s+SECURITY",
        )
    ...
    def test_upgrade_creates_instance_policy(self):
        ...
        self.assertRegex(
            self.src,
            r"CREATE\s+POLICY\s+memory_items_instance_isolation",
        )
        # Make sure we did NOT accidentally reuse the Wall-1 name.
        self.assertNotRegex(
            self.src,
            r"CREATE\s+POLICY\s+memory_items_tenant_isolation",
        )
    ...
    def test_downgrade_drops_policy(self):
        self.assertRegex(
            self.src,
            r"DROP\s+POLICY\s+IF\s+EXISTS\s+memory_items_instance_isolation",
        )

    def test_downgrade_drops_policy_but_does_not_disable_rls(self):
        # Arc 9 C5.4 fix: downgrade MUST NOT disable RLS, because the
        # sibling Wall-1 policy memory_items_tenant_isolation (C3.2b)
        # is still in force. Drop only this Wall-3 policy.
        disable_pattern = (
            r"ALTER\s+TABLE\s+memory_items\s+DISABLE\s+ROW\s+LEVEL\s+SECURITY"
        )
        drop_pattern = (
            r"DROP\s+POLICY\s+IF\s+EXISTS\s+memory_items_instance_isolation"
        )
        ...
        self.assertNotRegex(
            downgrade_block,
            disable_pattern,
            "C4.3c downgrade DISABLES RLS -- would neuter the Wall-1 "
            "sibling policy on memory_items.",
        )


class TestC43CMigrationImports(unittest.TestCase):
    def test_module_imports_cleanly(self):
        from alembic.script import ScriptDirectory
        from alembic.config import Config
        cfg = Config("/home/user/workspace/luciel/alembic.ini")
        sd = ScriptDirectory.from_config(cfg)
        rev = sd.get_revision("arc9_c4_3c_rls_instance_memory_items")
        self.assertIsNotNone(rev)
        self.assertEqual(rev.down_revision, "arc9_c4_3b_rls_instance_knowledge_embeddings")
