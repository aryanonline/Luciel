"""Arc 9 C3.5e -- RLS policy on admin_widget_domains (Wall 1 Layer 2).

Per-admin allowed-origin entries for embed widget CORS. Wall column is admin_id (NOT NULL, FK->admins.id). Strict RLS denies cross-admin reads/writes of CORS configuration.
...
down_revision = "arc9_c3_5d_rls_instances"
branch_labels = None
depends_on = None


def upgrade() -> None:
    op.execute("ALTER TABLE admin_widget_domains ENABLE ROW LEVEL SECURITY;")
    op.execute(
        """
        CREATE POLICY admin_widget_domains_tenant_isolation
        ON admin_widget_domains
        AS PERMISSIVE
        FOR ALL
        TO PUBLIC
...
def downgrade() -> None:
    op.execute(
        "DROP POLICY IF EXISTS admin_widget_domains_tenant_isolation ON admin_widget_domains;"
    )
    op.execute("ALTER TABLE admin_widget_domains DISABLE ROW LEVEL SECURITY;")
