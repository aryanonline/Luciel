"""Arc 9 C4.3a -- Wall 3 RLS policy on api_keys (luciel_instance_id).
...
materialises Wall 3's Layer 2 (PostgreSQL RLS) on api_keys.
...
revision = "arc9_c4_3a_rls_instance_api_keys"
down_revision = "arc9_c3_6b_rls_deletion_logs"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE POLICY api_keys_instance_isolation
        ON api_keys
        AS PERMISSIVE
        FOR ALL
        TO PUBLIC
...
def downgrade() -> None:
    ...
    op.execute(
        "DROP POLICY IF EXISTS api_keys_instance_isolation "
        "ON api_keys;"
    )
