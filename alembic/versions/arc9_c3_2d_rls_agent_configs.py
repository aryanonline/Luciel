"""Arc 9 C3.2d -- RLS policy on agent_configs (Wall 1 Layer 2).

Per-tenant agent persona/tool config.
...
revision = "arc9_c3_2d_rls_agent_configs"
...
def upgrade() -> None:
    op.execute("ALTER TABLE agent_configs ENABLE ROW LEVEL SECURITY;")
    op.execute(
        """
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE agent_configs DISABLE ROW LEVEL SECURITY;")
